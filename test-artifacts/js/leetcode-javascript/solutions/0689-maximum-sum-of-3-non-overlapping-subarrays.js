...
var maxSumOfThreeSubarrays = function(nums, k) {
    ...
    [maxFirst, maxFirstSecond, maxTotal] = [sumFirst, sumFirst + sumSecond, sumFirst + sumSecond + sumThird];
    for (let i = 1; i <= n - 3 * k; i++) {
        ...
        if (maxFirstSecond + sumThird > maxTotal) {
            [maxTotal, ...result] = [maxFirstSecond + sumThird, startFirstBest, startSecondBest, i + 2 * k];
        }
    }
    return result;
};