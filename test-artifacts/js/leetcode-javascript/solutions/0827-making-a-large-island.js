...
var largestIsland = function(grid) {
    function traverse(tile, grid, i, j) {
        if (i < 0 || j < 0 || i >= grid.length || j >= grid[0].length || grid[i][j] === 0 || grid[i][j] === tile) {
            return 0;
        }
        grid[i][j] = tile;
        return 1 + (traverse(tile, grid, i + 1, j) + traverse(tile, grid, i - 1, j) + traverse(tile, grid, i, j + 1) + traverse(tile, grid, i, j - 1));
    };
    const map = new Map();
    let tile = 2;
    let result = -1;
