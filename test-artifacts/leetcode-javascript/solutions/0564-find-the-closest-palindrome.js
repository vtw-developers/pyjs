...
var nearestPalindromic = function(n) {
    ...
    const candidates = [String(10n ** BigInt(n.length - 1) - 1n), createPalindrome(leftNum - 1n, n.length), createPalindrome(leftNum, n.length), createPalindrome(leftNum + 1n, n.length), String(10n ** BigInt(n.length) + 1n)].filter(x => x !== n);
    return candidates.reduce((min, curr) => {
        ...
        return currDiff < minDiff ? curr : currDiff === minDiff ? (curr < min ? curr : min) : min;
    });

    function createPalindrome(left, length) {
        const s = String(left);
        return length % 2 === 0 ? s + s.split('').reverse().join('') : s + s.slice(0, -1).split('').reverse().join('');
    }
};