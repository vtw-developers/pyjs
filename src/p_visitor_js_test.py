...
class TestLogStatementInserter(unittest.TestCase):
  def setUp(self):
    self.maxDiff = None
    self.src_lang = 'js'
    self.snippets_dir = p_consts.TEST_ARTIFACTS_DIR / 'js' / 'TestLogStatementInserter'
    self.parser = p_consts.PARSER_DICT[self.src_lang]

  def to_tree(self, snippet: str) -> p_visitor_js.Tree:
    ts_tree = self.parser.parse(bytes(snippet, 'utf-8'))
    tree = p_visitor_js.Tree.from_ts_tree(ts_tree)
    assert tree.root_node is not None
    assert tree.root_node.node_type == 'program'
    return tree

  def test_insert_log_statements_L0001(self):
    tree = self.to_tree(p_utils.read_text(self.snippets_dir / 'L0001_input.js'))
    inserter = p_visitor_js.LogStatementInserter(function_name='f_gold')
    inserter.visit(tree.root_node)
    pp_code = p_visitor_js.PrettyPrinter().visit(tree.root_node).strip()
    gold_code = p_utils.read_text(self.snippets_dir / 'L0001_output.js')
    self.assertEqual(pp_code, gold_code)
...
class TestAssignedIdentifierExtractor(unittest.TestCase):
  ...
  def extract_assigned_identifiers(self, snippet):
    ...
    extractor = p_visitor_js.AssignedIdentifierExtractor()
    extractor.visit(ast)
  ...
  def test_single_assignment(self):
    ...
    snippet = 'const num = 10;'
    assigned_identifiers = self.extract_assigned_identifiers(snippet)
    self.assertCountEqual(assigned_identifiers, ['num'])

    snippet = 'var z = 10;'
    assigned_identifiers = self.extract_assigned_identifiers(snippet)
    self.assertCountEqual(assigned_identifiers, ['z'])
  ...
  #   snippet = 'let x = 10, y = 20, z = 30;'
  #   assigned_identifiers = self.extract_assigned_identifiers(snippet)
  #   self.assertCountEqual(assigned_identifiers, ['x', 'y', 'z'])

  # def test_object_destructuring(self):
  #   # Test object destructuring
  #   snippet = 'const { a, b } = obj;'
  #   assigned_identifiers = self.extract_assigned_identifiers(snippet)
  #   self.assertCountEqual(assigned_identifiers, ['a', 'b'])

  # def test_array_destructuring(self):
  #   # Test array destructuring
  #   snippet = 'const [x, y] = arr;'
  #   assigned_identifiers = self.extract_assigned_identifiers(snippet)
  #   self.assertCountEqual(assigned_identifiers, ['x', 'y'])

  # def test_assignment_with_default_values(self):
  #   # Test assignments with default values
  #   snippet = 'const { a = 10, b = 20 } = obj;'
  #   assigned_identifiers = self.extract_assigned_identifiers(snippet)
  #   self.assertCountEqual(assigned_identifiers, ['a', 'b'])

  # def test_assignment_with_aliases(self):
  #   # Test assignments with aliases
  #   snippet = 'const { a: aliasA, b: aliasB } = obj;'
  #   assigned_identifiers = self.extract_assigned_identifiers(snippet)
  #   self.assertCountEqual(assigned_identifiers, ['aliasA', 'aliasB'])

  # def test_assignment_with_rest_operator(self):
  #   # Test assignments with the rest operator
  #   snippet = 'const [x, ...rest] = arr;'
  #   assigned_identifiers = self.extract_assigned_identifiers(snippet)
  #   self.assertCountEqual(assigned_identifiers, ['x', 'rest'])

  # def test_assignment_with_nested_rest_operator(self):
  #   # Test nested destructuring with the rest operator
  #   snippet = 'const { a, ...rest } = obj;'
  #   assigned_identifiers = self.extract_assigned_identifiers(snippet)
  #   self.assertCountEqual(assigned_identifiers, ['a', 'rest'])
