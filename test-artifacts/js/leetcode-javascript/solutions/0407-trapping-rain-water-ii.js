...
var trapRainWater = function(heightMap) {
    ...
    const history = new Array(heightMap.length).fill(0).map(() => new Array(heightMap[0].length).fill(false));
    ...
    while (minHeap.size()) {
        ...
        [
            ...
            if (rowIndex >= 0 && rowIndex < heightMap.length && columnIndex >= 0 && columnIndex < heightMap[0].length && !history[rowIndex][columnIndex]) {
                result += Math.max(0, weight - heightMap[rowIndex][columnIndex]);
                minHeap.enqueue([Math.max(weight, heightMap[rowIndex][columnIndex]), rowIndex, columnIndex]);
