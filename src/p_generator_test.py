...
class TestGenerateTspsWithGenerator(unittest.TestCase):
  ...
  def test_046_do_not_generate_strings(self):
  ...
  def test_046_01_do_not_generate_slices(self):
    template_dict = self.load_template_dict('046_01')
    self.log_ctx_prob_nodes(template_dict, '046_01')
    tsps = p_generator.generate_tsps_with_generator(template_dict)
    self.assertEqual(len(tsps), len(template_dict['tsps']))
    for tsp in tsps:
      for snippet in tsp:
  ...
  def test_060_return_str(self):
    template_dict = self.load_template_dict('060')
    self.log_ctx_prob_nodes(template_dict, '060')
    tsps = p_generator.generate_tsps_with_generator(template_dict)
    self.assertEqual(len(tsps), len(template_dict['tsps']))
    for tsp in tsps:
      for snippet in tsp:
