...
def grm_external_NT_pretty_string(grammar, external_state_id, external_NT):
  if d_consts.DEBUG_VERBOSE > -10: print("WARNING: grm_external_NT_pretty_string is not implemented. dumb value.")
...
def grm_preprocess(lang_prefix, grammar):
  ...
  d_utils.traverse_nested_list_and_dict(grammar["rules"], _find_transform_aliases_inner_fun)
  ...
  d_utils.traverse_nested_list_and_dict(grammar["rules"], _find_transform_immediate_string_inner_fun)
  ...
  if GRM_VERBOSE > 0: d_utils.traverse_nested_list_and_dict(grammar["rules"], _dbg_print_choice_aheads_inner_fun)

  # 12 file logging for debugging
  d_utils.log_json(lang_prefix + "_" + "grammar", grammar)
